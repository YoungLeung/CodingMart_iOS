...
- (void)setCurPriMsg:(EAChatMessage *)curPriMsg andPrePriMsg:(EAChatMessage *)prePriMsg{
    ...
    [_userIconView sd_setImageWithURL:[_curPriMsg.sender.avatar urlImageWithCodePathResizeToView:_userIconView] placeholderImage:[UIImage imageNamed:@"placeholder_user"]];
...
- (void)configSendStatus{
    ...
    }else if (_curPriMsg.sendStatus == EAChatMessageSendStatusIng){
        if (!_sendingStatus) {
    ...
    }else if (_curPriMsg.sendStatus == EAChatMessageSendStatusFail){
        if (_sendingStatus) {
            [_sendingStatus stopAnimating];
        }
        __weak typeof(self) weakSelf = self;
...
+ (CGFloat)cellHeightWithObj:(id)obj preObj:(id)preObj{
    ...
    if ([obj isKindOfClass:[EAChatMessage class]]) {
        EAChatMessage *curPriMsg = (EAChatMessage *)obj;
        ...
        EAChatMessage *prePriMsg = (EAChatMessage *)preObj;
...
+ (NSString *)displayTimeStrWithCurMsg:(EAChatMessage *)cur preMsg:(EAChatMessage *)pre{
...
+ (CGFloat)mediaViewHeightWithObj:(EAChatMessage *)curPriMsg{
